...
app.get("/v1/fizzbuzz/:number", (request, response) => {
    ...
    const clientNumber = ExplorerController.getClientNumber(number);
    response.json({score: request.params.number, trick: clientNumber});
});

app.get("/v1/explorers/stack/:stack", (request, response) => {
    const stack = request.params.mission;
